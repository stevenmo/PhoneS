...
@property (weak, nonatomic) IBOutlet PinchScrollView *pinchScrollView;
...
- (void)viewDidLoad
{
    ...
    self.pinchScrollView.maximumZoomScale = 4.0;

    self.previewView.frame = CGRectMake(0,0,self.view.frame.size.width,self.view.frame.size.height);
	self.previewView.session = self.session;

    [ Utils setupLongPressHandlerOnView: self.previewView target:self sel:@selector(onPreviewLongClicked:) duration: 3.0 ];
    ...
    if ( __setFrontCamera && ![self isCurrentFrontCamera ] )
...
-(void) stop
{
    if ( self.setupResult == AVCamSetupResultSuccess ) {
        ...
        self.setupResult = AVCamSetupEnd;
    }
}
...
- (void) onPreviewLongClicked: (id) sender
{
    NSLog( @"onPreviewLongClicked" );
    if ( __delegate != nil ) {
        [ __delegate onPlayEnd ];
    }
}
...
- (IBAction)changeCamera:(id)sender
{
	...
	dispatch_async( self.sessionQueue, ^{
		...
		if ( [self.session canAddInput:videoDeviceInput] ) {
			...
			[AAPLCameraViewController setFlashMode:_flashMode forDevice:videoDevice];
...
- (IBAction)snapStillImage:(id)sender
{
	dispatch_async( self.sessionQueue, ^{
		...
		[AAPLCameraViewController setFlashMode:_flashMode forDevice:self.videoDeviceInput.device];
